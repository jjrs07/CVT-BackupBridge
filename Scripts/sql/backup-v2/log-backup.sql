/*
    CVT BackupBridge v2 - LOG backup example
...
BEGIN
    THROW 51004, 'Transaction-log backup requires the FULL recovery model for this BackupBridge workflow.', 1;
END;
...
BEGIN
    THROW 51005, 'No conventional full backup was found in msdb. Take a non-COPY_ONLY full backup before log backups.', 1;
END;
...
DECLARE @Directory nvarchar(2048) =
    @BackupRoot + N'\' + @ServerComponent + N'\' + @DatabaseComponent + N'\LOG';
DECLARE @FileName nvarchar(512) =
    @ServerComponent + N'_' + @DatabaseComponent + N'_LOG_'
    + @Timestamp + N'Z_' + @UniqueSuffix + N'.trn';
DECLARE @BackupFile nvarchar(2600) = @Directory + N'\' + @FileName;
DECLARE @BackupName nvarchar(128) =
    @DatabaseName + N' LOG backup'
    ;
DECLARE @Description nvarchar(255) =
    N'CVT BackupBridge v2 LOG backup; UTC='
    + CONVERT(nvarchar(33), @UtcNow, 126) + N'Z';
DECLARE @Sql nvarchar(max);

PRINT N'BackupType: LOG';
